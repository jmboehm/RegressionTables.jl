...
function write_xlsx(render, ws, cell, val::Union{AbstractUnderStatistic, AbstractRegressionStatistic}; fmt=xlsx_format(render, val), vargs...)
    v = value(val)
    write_xlsx(render, ws, cell, v; fmt, vargs...)
end

function write_xlsx(render, ws, cell, val::CoefValue; fmt=xlsx_format(render, val), vargs...)
